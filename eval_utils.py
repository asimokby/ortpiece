...
#     preds_filt = [p for p in preds if p['image_id'] in valids]
    ...
    with open('data/tr_eval_500_gts.json', 'r') as fp: # using the turkish evalset here for evaluation instead of the default english coco ones. 
        gts = json.load(fp)
    
    cocoRes = coco.loadRes(cache_path)
    cocoEval = CorrectCOCOEvalCap(coco, cocoRes, gts_values=gts)
...
def decode_tokenized_sent(sent):
    decoded_tokens = []
...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    is_wordpiece = eval_kwargs.get('is_wordpiece', 1)
    ...
    while True:
        ...
        if beam_size > 1 and verbose_beam:
            for i in range(loader.batch_size):
                print('\n'.join([utils.decode_sequence(is_wordpiece, loader.get_vocab(), _['seq'].unsqueeze(0))[0] for _ in model.done_beams[i]]))
                print('--' * 10)
        sents = utils.decode_sequence(is_wordpiece, loader.get_vocab(), seq)

        for k, sent in enumerate(sents):
            image_id = data['infos'][k]['id']
            print('this is me', '*' * 100)
            print(sent.split())
            print(image_id)
            print('this is me', '*'* 100)
            entry = {'image_id': image_id, 'caption': decode_tokenized_sent(sent),
