...
def train(opt):
    ...
    epoch_loss_history = histories.get('epoch_loss_history', {})
...
#         if (iteration % opt.save_checkpoint_every == 0):
        if epoch_done:
            ...
            val_result_history[epoch] = {'loss': val_loss, 'lang_stats': lang_stats, 'predictions': predictions}
            epoch_loss_history[epoch] = train_loss if not sc_flag else np.mean(reward[:,0])
            ...
            if True: # if true
                ...
                histories['epoch_loss_history'] = epoch_loss_history
        ...
        if patience >= 5:
            print("Reached patience limit for early-stopping")
            break
