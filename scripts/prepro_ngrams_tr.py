...
def get_tokenizer():
    from_ = 'data/wordpiece_tokenizer.json'    
...
def build_dict(imgs, wtoi, params, tokenizer):
  wtoi['<eos>'] = 0
  ...
  for img in imgs:
    if (params['split'] == img['split']) or \
      ...
      for sent in img['sentences']:
        tokenized_tokens = tokenizer.tokenize(sent['raw'])
        tmp_tokens = tokenized_tokens + ['<eos>']
...
def main(params):
  ...
  ngram_words, ngram_idxs, ref_len = build_dict(imgs, wtoi, params, tokenizer)
...
#   cPickle.dump({'document_frequency': ngram_idxs, 'ref_len': ref_len}, open(params['output_pkl']+'-idxs.p','wb'), protocol=cPickle.HIGHEST_PROTOCOL)
  ...
  joblib.dump({'document_frequency': ngram_words, 'ref_len': ref_len}, params['output_pkl']+'-words.p', protocol=2)
  joblib.dump({'document_frequency': ngram_idxs, 'ref_len': ref_len}, params['output_pkl']+'-idxs.p', protocol=2)
