...
def main(params):
  ...
  for i,img in enumerate(imgs):
    ...
    # write to pkl
    np.save(os.path.join(dir_fc, str(img[params['imgid_key_name']])), tmp_fc.data.cpu().float().numpy())
    np.savez_compressed(os.path.join(dir_att, str(img[params['imgid_key_name']])), feat=tmp_att.data.cpu().float().numpy())
...
if __name__ == "__main__":
  ...
  parser.add_argument('--imgid_key_name', default='cocoid', help='the name by which the features are saved which is the key of each image')
